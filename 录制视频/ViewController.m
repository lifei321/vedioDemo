...
- (void)clickClearMemory{
   ...
   NSString *path = [LBClearCacheTool getCacheSizeWithFilePath: [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject]];
    ...
    UIAlertAction *actionOk=[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
        ...
        //清楚缓存
        BOOL isSuccess = [LBClearCacheTool clearCacheWithFilePath:[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject]];
        if (isSuccess) {
...
{
    [MBProgressHUD showSuccess:@"录制完成"];
...
- (void)mergeAndExportVideos:(NSArray*)videosPathArray withOutPath:(NSString*)outpath{
    if (videosPathArray.count == 0) {
        [MBProgressHUD showError:@"请录制视频"];
        return;
    }
    
    [MBProgressHUD showMessage:@"合并中..." toView:self.view];
    ...
    [exporter exportAsynchronouslyWithCompletionHandler:^{
        ...
        if (exporter.error != nil) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [MBProgressHUD showSuccess:@"合并失败"];
            });
            NSLog(@"合并失败");
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [MBProgressHUD showSuccess:@"合并成功"];
            });
